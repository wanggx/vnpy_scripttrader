...
def get_trading_dates(end_date: str) -> list[str]:
    """获取起始日期至结束日期之间的沪市交易日。"""
    try:
...
def get_stock_open_dates(stock_codes: list[str]) -> dict[str, str]:
...
def run(engine: "ScriptEngine") -> None:
    ...
    trading_dates: list[str] = get_trading_dates(end_date)
    ...
    open_dates: dict[str, str] = get_stock_open_dates(stock_codes)
